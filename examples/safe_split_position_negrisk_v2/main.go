...
func main() {
	...
	if amountStr == "" {
		log.Fatal("AMOUNT environment variable is required in raw units (e.g., '1000000' for 1 pUSD)")
	}
	amount := new(big.Int)
	...
	log.Println("\n=== V2 Split NegRisk Position via Safe (NegRiskCtfCollateralAdapter) ===")
	log.Printf("Condition ID: %s", conditionId.Hex())
	log.Printf("Amount: %s pUSD", amountStr)
	log.Println("Partition: [1, 2] (binary Yes/No)")
	log.Println("\nThis splits pUSD into NegRisk conditional tokens via the NegRiskCtfCollateralAdapter.")
	log.Println("Use this for NegRisk markets only. For standard markets, use safe_split_position_v2.")

	var condID [32]byte
	copy(condID[:], conditionId[:])

	txHash, err := v2.SplitPositionNegRiskForSafe(ctx, safeSigner, polygonChainID, condID, partition, amount)
	if err != nil {
		log.Fatalf("Failed to split NegRisk position: %v", err)
	}
