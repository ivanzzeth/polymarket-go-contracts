...
func main() {
	...
	log.Println("\n=== Unwrapping pUSD via Safe ===")
	log.Printf("Asset (target):  %s", asset.Hex())
	log.Printf("Recipient:       %s", safeAddr.Hex())
	log.Printf("Amount:          %s tokens (%s human)", amount.String(), amountStr)

	txHash, err := v2.UnwrapFromPUSDForSafe(ctx, safeSigner, polygonChainID, asset, safeAddr, amount)
	if err != nil {
		log.Fatalf("Failed to unwrap pUSD: %v", err)
	}
