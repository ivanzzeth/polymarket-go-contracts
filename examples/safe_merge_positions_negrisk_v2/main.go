...
func main() {
	...
	log.Println("\n=== V2 Merge NegRisk Positions via Safe (NegRiskCtfCollateralAdapter) ===")
	log.Printf("Condition ID: %s", conditionId.Hex())
	log.Printf("Amount: %s tokens", amountStr)
	log.Println("Partition: [1, 2] (binary Yes/No)")
	log.Println("\nThis merges NegRisk conditional tokens back into pUSD via the NegRiskCtfCollateralAdapter.")
	log.Println("You must hold ALL outcome tokens to merge successfully.")
	log.Println("Use this for NegRisk markets only. For standard markets, use safe_merge_positions_v2.")

	var condID [32]byte
	copy(condID[:], conditionId[:])

	txHash, err := v2.MergePositionsNegRiskForSafe(ctx, safeSigner, polygonChainID, condID, partition, amount)
	if err != nil {
