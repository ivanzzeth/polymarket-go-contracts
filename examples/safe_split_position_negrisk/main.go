...
func main() {
	...
	log.Println("\n=== Splitting NegRisk Position via Safe ===")
	log.Printf("Condition ID: %s", conditionId.Hex())
	log.Printf("Amount: %s USDC", amountStr)
	log.Println("\nThis will split your USDC into conditional tokens (outcome tokens) for a NegRisk market.")
	log.Println("⚠️  Note: This is for NegRisk markets only. Use safe_split_position for standard markets.")

	// Split position through Safe using NegRisk adapter
	txHash, err := polymarketInterface.SplitNegRisk(ctx, conditionId, amount)
	if err != nil {
		log.Fatalf("Failed to split NegRisk position: %v", err)
	}

	log.Println("\n✅ NegRisk position split successfully!")
	log.Printf("Safe Address: %s", safeAddr.Hex())
	...
	log.Println("\n📝 What happened:")
	log.Printf("   • %s USDC was locked in the CTF contract via NegRiskAdapter", amountStr)
