...
func main() {
	...
	log.Println("\n=== V2 Split Position via Safe (CtfCollateralAdapter) ===")
	log.Printf("Condition ID: %s", conditionId.Hex())
	log.Printf("Amount: %s pUSD", amountStr)
	log.Println("Partition: [1, 2] (binary Yes/No)")
	log.Println("\nThis splits pUSD into conditional tokens via the CtfCollateralAdapter.")

	var condID [32]byte
	copy(condID[:], conditionId[:])

	txHash, err := v2.SplitPositionForSafe(ctx, safeSigner, polygonChainID, condID, partition, amount)
	if err != nil {
